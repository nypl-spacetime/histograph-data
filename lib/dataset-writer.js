...
module.exports = function (schemas, etlConfig, dataset, dir, meta) {
  const validators = {
  ...
  let dataValidator
  if (meta.schema) {
    dataValidator = createValidator(meta.schema)
  }
  ...
  function writeObject (data, callback) {
    ...
    let validate = true
    if (etlConfig.validate !== undefined && !etlConfig.validate) {
      validate = false
    }

    if (streamName !== 'logs' && validate) {
